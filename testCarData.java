...
class testCarData {
	public static void main(String[] args) {
		if (args.length != 5) {
			System.out.println(
					"format testCarData \"input file\" \"min total range\" \"max total range\" \"min remaining range\" \"max remaining range\"");
			System.exit(0);
		}
		...
		ManageCarDataFunctions manageCarData = new vural_ManageCarData();
		...
		// get the list of cars stored in the PriorityQueue ordered by total range via
		// an iterator and print it out
		...
		// get an iterator for the PriorityQueue ordered by total range and print them
		// out
		...
		// get the list of cars stored in the PriorityQueue ordered by remaining range
		// via an iterator and print it out
		System.out.println("carListByRemainingRange iterator");
		ArrayList<CarFunctions> carListByRemainingRangeByIterator = manageCarData
				.getCarListByRemainingRangeUsingIterator();
		for (CarFunctions c : carListByRemainingRangeByIterator) {
			System.out.println(c);
		}
		System.out.println();

		// get an iterator for the PriorityQueue ordered by remaining range and print
		// them out
		...
		// get the list of cars stored in the PriroityQueue ordered by total range
		// having total range [minTotalRange, maxTotalRange]
		System.out.println(
				"carListByTotalRange.poll().getTotalRangeInMiles() in [" + minTotalRange + "," + maxTotalRange + "]");
		ArrayList<String> carListByTotalRangeByPoll = manageCarData.getCarListByTotalRangeViaPoll(minTotalRange,
				maxTotalRange);
		...
		// get the list of cars stored in the PriroityQueue ordered by remaining range
		// having total range [minRemainingRange, maxRemainingRange]
		System.out.println("carListByTotalRange.poll().getRemainingRangeInMiles() in [" + minRemainingRange + ","
				+ maxRemainingRange + "]");
		ArrayList<String> carListByRemainingRangeByPoll = manageCarData
				.getCarListByRemainingRangeViaPoll(minRemainingRange, maxRemainingRange);
